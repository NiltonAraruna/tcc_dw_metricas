...
entrevistas, questionários e registro de observação na vida real. A seleção metodológica é apresentada na Figura \ref{7eixosqualidade} e a descrição das classificações adotadas se encontra nas Tabelas \ref{tab:descricaoMetodologiaa1} e \ref{tab:descricaoMetodologiaa2}.
...
\label{tab:descricaoMetodologiaa1}
\end{center}
\end{table}	
\FloatBarrier
...
\label{tab:descricaoMetodologiaa2}
\end{center}
\end{table}	
\FloatBarrier
...
\cite{wohlin2012experimentation} fraciona o estudo de caso em cinco passos. Nesta pesquisa, o estudo de caso compreende os passos: Planejamento do Estudo de Caso; Coleta dos Dados; Análise dos Dados e Compartilhar os Resultados. Portanto, os passos Projetar o Estudo de Caso e Preparar a Coleta de Dados definidas por \citeonline{wohlin2012experimentation} foram agrupadas no passo Planejamento do Estudo de Caso como na Figura \ref{passo Estudo de Caso}.
...
O passo Planejamento do Estudo de Caso consiste na determinação do objetivo e da questão de pesquisa, da escolha da metodologia de pesquisa, da definição das fases da pesquisa, da definição do procedimentos de pesquisa, do protocolo, das técnicas de coleta de dados e da proposta do trabalho final.

No passo Coleta dos Dados são executados os procedimentos de pesquisa e as técnicas de coletas de dados a seguir:
...
& Documentos: coleta de dados dos documentos dos processos fornecidos pelo Órgãos público do estudo de caso será realizada para coleta de dados para análise, e

& Observação na Vida Real: coleta de dados a partir da observação dentro do Órgão Público.

\end{easylist}

O passo Análise dos Dados é onde os dados coletados serão analisados e interpretados. A análise compreende tanto a análise quantitativa quanto a análise qualitativa.

Por fim, o passo Compartilhar os resultados diz respeito a expor os resultados de forma adequada para o leitor alvo.

\section{Metodologia de Desenvolvimento}

A metodologia de desenvolvimento utilizada no Trabalho de Conclusão de Curso 1 foi baseada em reuniões quinzenais junto ao orientador cujo objetivo era discutir sobre o tema, levantar as necessidades, atribuir atividades para suprir as necessidades e validar os resultados das atividades já realizadas. Ao final do Trabalho de Conclusão de Curso 1 foi elaborado um cronograma com as macro atividades para serem realizadas durante a o Trabalho de Conclusão de Curso 2. Contudo, após a defesa do Trabalho de Conclusão de Cursos 1, houve um realinhamento sobre os objetivos esperados, o que resultou na adequação do planejamento inicial.

Objetivando maximizar a produtividade, as reuniões passaram a ser semanais e começou a ser utilizada a ferramenta \textit{kanbanFlow} para gerir tarefas usando o método \textit{Kanban}. O \textit{Kanban} foi dividido em 4 colunas como mostra a Figura \ref{kanban} :

\begin{easylist}[itemize]

& \textbf{Main Backlog:} Onde ficam todas as atividades que devem ser realizadas durante todo o Trabalho de Conclusão de Curso 2;

& \textbf{Sprint Backlog:} Onde ficam as atividades que devem ser realizadas durante a semana(\textit{Sprint});

& \textbf{In Progress:} Onde ficam as atividades que estão em andamento, e
...
O Apêndice \ref{sec:kanban} contém um relatório que demonstra as atividades realizadas durante todo o Trabalho de Conclusão de Curso 2 e as horas gastas por atividade divididas pelas colunas descritas acima.
